...
SUITE(NodeLoadingTests)
{
	...
	TEST_FIXTURE(MyFixture, LoadingBadValues)
	{
			...
			try
			{
				node.poll("you wont find this", dummyInts);
				CHECK(false);
			} catch(std::runtime_error)
			{
				CHECK(true);
			...
			try
			{
				node.poll("floats", dummyFloats, 7);
				CHECK(false);
			} catch(std::runtime_error)
			{
				CHECK(true);
			...
			try
			{
				node.poll("floats", dummyFloats, 9);
				CHECK(false);
			} catch(std::runtime_error)
			{
				CHECK(true);
